...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        new_actions = set()
        for action in self.problem.actions_list + self.noop_actions(self.problem.state_map):
            a_node = PgNode_a(action)
            if a_node.prenodes < self.s_levels[level]:
                a_node.parents = self.s_levels[level]
                new_actions.add(a_node)
    ...
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[level - 1]:
            for future in action.effnodes:
                s_node = PgNode_s(future.symbol, future.is_pos)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        if node_a1.action.effect_add == node_a2.action.precond_neg or "Noop" in node_a2.action.name or "Noop" in node_a1.action.name:
            return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for action in node_a1.parents:
            for action2 in node_a2.parents:
                if action.is_mutex(action2):
                    return True
        return False
