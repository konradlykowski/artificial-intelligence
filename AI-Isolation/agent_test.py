...
class Project1Test(unittest.TestCase):
    def initAUTMinimax(self, depth, eval_fn, iterative=False,
                       method="minimax", loc1=(3, 3), loc2=(0, 0), w=7, h=7):
        """Generate and initialize player and board objects to be used for
        testing.
        """
        reload(game_agent)
        agentUT = game_agent.MinimaxPlayer(score_fn=eval_fn)
        board = CounterBoard(agentUT, 'null_agent', w, h)
        board.apply_move(loc1)
        board.apply_move(loc2)
        return agentUT, board

    def initAUTAlphaBeta(self, depth, eval_fn, iterative=False,
                         method="minimax", loc1=(3, 3), loc2=(0, 0), w=7, h=7):
        """Generate and initialize player and board objects to be used for
        testing.
        """
        reload(game_agent)
        agentUT = game_agent.AlphaBetaPlayer(score_fn=eval_fn)
        board = CounterBoard(agentUT, 'null_agent', w, h)
        board.apply_move(loc1)
        board.apply_move(loc2)
        return agentUT, board
    ...
    def test_minimax_interface(self):
        ...
        agentUT = game_agent.MinimaxPlayer(score_fn=heuristic)
    ...
    def test_alphabeta_interface(self):
        ...
        # create a player agent & a game board
        agentUT = game_agent.AlphaBetaPlayer(score_fn=heuristic)
        agentUT.time_left = lambda: 99  # ignore timeout for fixed-depth search
        board = isolation.Board(agentUT, 'null_agent', w, h)
    ...
    def test_minimax(self):
        ...
        for idx in range(5):
            test_depth = idx + 1
            agentUT, board = self.initAUTMinimax(test_depth, heuristic,
    ...
    def test_alphabeta(self):
        ...
        for idx in range(len(counts)):
            ...
            agentUT, board = self.initAUTAlphaBeta(test_depth, heuristic,
                                          iterative_search, method,
                                          loc1=starting_location,
                                          loc2=adversary_location,
                                          w=w, h=h)
    ...
    def test_get_move(self):
        ...
        for idx in range(len(origins)):
            ...
            agentUT, board = self.initAUTAlphaBeta(-1, eval_fn, True, method,
